...
export async function buildChecks(
  ...
  if (engine.kind === 'pglite') {
    checks.push({
      name: 'pgvector',
      status: 'ok',
      message: 'Skipped (PGLite — pgvector is provided by the embedded WASM engine)',
    });
  } else {
    try {
      const sql = db.getConnection();
  ...
  if (engine.kind === 'pglite') {
    checks.push({
      ...
      status: 'ok',
      message: 'Skipped (PGLite stores JSON values locally; the double-encoded JSONB failure mode is a Postgres/Supabase check)',
    });
  } else {
    try {
      const sql = db.getConnection();
